...
namespace ContactManager.UI.WinForms.Forms
{
    ...
    public partial class CustomerListForm : BaseForm
    {
        ...
        private void BtnCSVExport_Click(object? sender, EventArgs e)
        {
            ...
            try
            {
                List<Customer> all = _contacts.Customers.GetAll().ToList();
                CsvContactConverter.ExportCustomers(all, dialog.FileName);

                MessageBox.Show($"{all.Count} Kunden wurden exportiert.",
        ...
        // Importiert Kunden aus einer CSV-Datei. Jede gültig gelesene Zeile durchläuft
        // dieselbe Validierung und Nummernvergabe wie eine manuelle Erfassung
        // (contacts.Customers.Add) - der Import umgeht also keine Geschäftsregel.
        // Fehlerhafte Zeilen werden übersprungen statt den ganzen Import abzubrechen; am
        // Schluss steht eine zusammenfassende Meldung, analog zum Löschen mehrerer Kunden
        ...
        private void BtnCSVImport_Click(object? sender, EventArgs e)
        {
            ...
            CsvImportResult<Customer> parsed;

            try
            {
                parsed = CsvContactConverter.ImportCustomers(dialog.FileName);
            }
            ...
            foreach (CsvImportRow<Customer> row in parsed.Imported)
            {
                try
                {
                    _contacts.Customers.Add(row.Item);
                    imported++;
                }
            ...
            MessageBox.Show(
                CsvContactConverter.BuildImportSummary(imported, "Kunde/in", "Kunden", errors, aborted),
